...
def plot_grouped_accuracy_bar_region(df_region, region, save_path):
    ...
    bars1 = plt.bar(x - width/2, low_means, width, yerr=low_sems, capsize=4, label='Low Engagement', color="#008BFB", alpha=0.9, edgecolor='black', linewidth=1.2)
    bars2 = plt.bar(x + width/2, high_means, width, yerr=high_sems, capsize=4, label='High Engagement', color="#FF8C00", alpha=0.9, edgecolor='black', linewidth=1.2)
    
    plt.ylabel('Decoding Accuracy', fontsize=12)
    plt.title(f'[{region}] Average Decoding Accuracy (n={n_sess} sessions)', fontsize=14, fontweight='bold')
