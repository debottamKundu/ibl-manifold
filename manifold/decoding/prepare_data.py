...
def prepare_ephys(
    one, session_id, pids, pnames, regions, intervals, qc=1, min_units=10, stage_only=False
):
    ...
    to_merge = [
        load_good_units(one, pid, qc=qc, pname=pname, eid=session_id)
    ...
    if isinstance(regions, str):
        ...
        if (
            ...
            regions == "all_regions"
        ):  # if all regions make a list of a single list to decode all regions together
            regions = [[b for b in np.unique(beryl_regions) if b not in ["root", "void"]]]
        else:
    ...
    for region in regions:
        ...
        if sum(region_mask) < min_units:
            print(
                f"{'_'.join(region)} below min units threshold ({min_units}) : {sum(region_mask)}, not decoding"
            )
        else:
...
def prepare_motor(
    one, session_id, align_event="stimOn_times", time_window=(-0.6, -0.1), lick_bins=0.02
):
    ...
    lick_times = [
        get_licks(sl.pose[f"{side}Camera"], sl.pose[f"{side}Camera"]["times"])
    ...
    nose_epochs = average_data_in_epoch(
        sl.pose["leftCamera"]["times"],
        sl.pose["leftCamera"]["nose_tip_x"],
        sl.trials,
    ...
    whisking_l_epochs = average_data_in_epoch(
        sl.motion_energy["leftCamera"]["times"],
        sl.motion_energy["leftCamera"]["whiskerMotionEnergy"],
        sl.trials,
        align_event=align_event,
        epoch=time_window,
    )
    whisking_r_epochs = average_data_in_epoch(
        sl.motion_energy["rightCamera"]["times"],
        sl.motion_energy["rightCamera"]["whiskerMotionEnergy"],
        sl.trials,
        align_event=align_event,
        epoch=time_window,
    )
    ...
    paw_pos_r = (
        sl.pose["rightCamera"][["paw_r_x", "paw_r_y"]]
        .pow(2)
        .sum(axis=1, skipna=False)
        .apply(np.sqrt)
    )
    paw_pos_l = (
        sl.pose["leftCamera"][["paw_r_x", "paw_r_y"]]
        .pow(2)
        .sum(axis=1, skipna=False)
        .apply(np.sqrt)
    )
...
def prepare_pupil(
    one, session_id, time_window=(-0.6, -0.1), align_event="stimOn_times", camera="left"
):
    # Load the trials data
    sl = SessionLoader(one, session_id)
    sl.load_trials()
    # TODO: replace this with SessionLoader ones that loads lightning pose
    pupil_data = one.load_object(
        session_id, f"{camera}Camera", attribute=["lightningPose", "times"]
    )
...
def prepare_widefield(
    ...
    channel_info = one.load_dataset(
        session_id, "imagingLightSource.properties.htsv", download_only=True
    )
    channel_info = pd.read_csv(channel_info)
    lmark_file = one.load_dataset(
        session_id, "widefieldLandmarks.dorsalCortex.json", download_only=True
    )
    ...
    functional_chn = channel_info.loc[channel_info["wavelength"] == functional_channel][
        "channel_id"
    ].values[0]
    ...
    intervals = (
        np.sort(frames[:, None] + np.arange(frame_window[0], frame_window[1] + 1), axis=1)
        .astype(int)
        .squeeze()
    )

    # Get the brain regions for now disregarding hemisphere
    atlas_regions = ccf_regions.acronym[
        ccf_regions.label.isin(np.unique(downsampled_atlas))
    ].values
    if isinstance(regions, str):
        if (
            regions in config["region_defaults"].keys()
...
def prepare_widefield_old(old_data, hemisphere, regions, align_event, frame_window):
    ...
    downsampled_stack = np.load(
        old_data.joinpath("activity.npy")
    )  # used to be neural_activity['activity']
    frames = pd.read_parquet(
        old_data.joinpath("timings.pqt")
    )  # used to be neural_activity['timings']
    downsampled_atlas = np.load(
        old_data.joinpath("regions.npy")
    )  # used to be neural_activity['regions']
    ccf_regions, _, _ = wfield.allen_load_reference("dorsal_cortex")
    ...
    intervals = (
        np.sort(
            frames[align_event].values[:, None] + np.arange(frame_window[0], frame_window[1] + 1),
            axis=1,
        )
        .astype(int)
        .squeeze()
