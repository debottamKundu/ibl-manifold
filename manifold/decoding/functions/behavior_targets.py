...
from .utils import check_config_decoding, check_bhv_fit_exists
...
def optimal_Bayesian(act, side):
    ...
    b[0][0][-1], b[0][-1][0], b[0][1][np.array([0, 2])] = 1, 1, 1.0 / 2  # case when l_t = 1
...
def compute_beh_target(
    ...
    signal = model.compute_signal(
        signal="prior" if target == "pLeft" else target, act=actions, stim=stimuli, side=stim_side
    )["prior" if target == "pLeft" else target]
    tvec = signal.squeeze()
