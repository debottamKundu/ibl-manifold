...
from .behavior_targets import compute_beh_target
from .utils import check_config_decoding
...
def compute_neurometric_prior(trials_df, session_id, subject, model, behavior_path):
    if model != "oracle":
        blockprob_neurometric = compute_beh_target(
            trials_df, session_id, subject, model, "pLeft", behavior_path
        )
...
def get_neurometric_parameters_(prob_arr, possible_contrasts, nfits=100):
    ...
    if len(prob_arrs) > 2:
        for k in range(1, len(prob_arrs) - 1):
            ...
            params = {
                **params,
                **{
                    "quantile_%i_0contrastLevel" % k: mediums["fit_trace"][mediums["zerind"]],
...
def get_neurometric_parameters(fit_result, trialsdf, compute_on_each_fold):
    ...
    if compute_on_each_fold:
        try:
            ...
            fold_neurometric = [
                fit_get_shift_range(prob_arrays[k][0], prob_arrays[k][1])
                for k in range(fit_result["n_folds"])
            ]
        except KeyError:
    ...
    full_neurometric = fit_get_shift_range(prob_arrs, nfits=100)
